...
// CreateWebhook creates a new webhook for GitHub repository
func (repo *GitHubRepository) CreateWebhook(listenerURL, secret string) (string, error) {
	return repo.Client.CreateWebhook(repo.path, listenerURL, secret)
}

// ListWebhooks returns a list of webhook IDs of the given listener in GitHub repository
func (repo *GitHubRepository) ListWebhooks(listenerURL string) ([]string, error) {
	return repo.Client.ListWebhooks(repo.path, listenerURL)
}

// DeleteWebhooks deletes all webhooks that associate with the given listener in this GitHub repository
func (repo *GitHubRepository) DeleteWebhooks(ids []string) ([]string, error) {
	return repo.Client.DeleteWebhooks(repo.path, ids)
}
