...
func CreateInternalRegistryResources(env *config.Pipeline, sa *corev1.ServiceAccount, imageRepo string) ([]string, res.Resources, error) {
	...
	namespacePath := filepath.Join(config.PathForPipelineConfig(env), "base", "pipelines", filename)
...
func createInternalRegistryRoleBinding(env *config.Pipeline, ns string, sa *corev1.ServiceAccount) (string, res.Resources) {
	...
	roleBindingPath := filepath.Join(config.PathForPipelineConfig(env), "base", "pipelines", roleBindingFilname)
