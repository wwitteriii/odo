...
func TestValidate(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			multierror.Join([]error{
				missingFieldsError([]string{"services", "config_repo"}, []string{"environments.development.apps.app-1"}),
				missingFieldsError([]string{"path"}, []string{"environments.development.apps.app-2.config_repo"}),
		...
		},
		{
			...
			multierror.Join(
				[]error{
					missingServiceRefError("app-1-service-metrics", "app-1", []string{"environments.duplicate-service.apps.app-1"}),
				},
			),
		},
		{
