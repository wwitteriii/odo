...
func bootstrapServiceDeployment(dev *config.Environment, appName string) (res.Resources, error) {
	...
	resources[filepath.Join(svcBase, "200-service.yaml")] = createBootstrapService(dev.Name, svc.Name, appName)
...
func createBootstrapService(ns, name, appName string) *corev1.Service {
	svc := &corev1.Service{
